...
const AdminDashboard = () => {
  const { data: stats, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [users, courses, orders, access, categories] = await Promise.all([
        supabase.from("profiles").select("id, created_at", { count: "exact", head: false }).order("created_at", { ascending: false }).limit(5),
  ...
  const cards = [
    ...
    { label: "Активные подписки", value: stats?.activeAccess || 0, icon: CreditCard, color: "text-success" },
    { label: "Курсов / программ", value: stats?.totalCourses || 0, icon: BookOpen, color: "text-accent" },
    { label: "Заказов", value: stats?.totalOrders || 0, icon: Receipt, color: "text-warning" },
    { label: "Категорий", value: stats?.totalCategories || 0, icon: TrendingUp, color: "text-primary" },
    { label: "Выручка", value: `${new Intl.NumberFormat("ru-RU").format(stats?.totalRevenue || 0)} ₽`, icon: Receipt, color: "text-success" },
  ];
        ...
        <div className="rounded-xl border border-border bg-card">
          ...
          <div className="divide-y divide-border">
            {(stats?.recentOrders || []).slice(0, 5).map((o: any) => (
              <div key={o.id} className="p-4 flex items-center justify-between text-sm">
                ...
                <div className="flex items-center gap-2">
                  <span>{new Intl.NumberFormat("ru-RU").format(Number(o.amount))} ₽</span>
                  <Badge className={statusCls[o.payment_status] || "bg-muted text-muted-foreground"}>
                    {o.payment_status}
                  </Badge>
                </div>
              </div>
            ...
            {(!stats?.recentOrders || stats.recentOrders.length === 0) && (
              <p className="p-4 text-sm text-muted-foreground">Нет заказов</p>
            )}
          </div>
        </div>
        ...
        <div className="rounded-xl border border-border bg-card">
          ...
          <div className="divide-y divide-border">
            ...
            {(!stats?.recentUsers || stats.recentUsers.length === 0) && (
              <p className="p-4 text-sm text-muted-foreground">Нет пользователей</p>
            )}
          </div>
        </div>
      </div>
